...
export default function Home() {
  ...
  const calculateKPIMetrics = (
    ...
    return {
      ...
      clientVisits,
      staffUtilization,
      appointmentChange: calculateChange(totalAppointments, yesterdayTotalAppointments),
      ...
      utilizationChange: calculateChange(staffUtilization, yesterdayStaffUtilization),
    };
  };

  // NEW: Function to fetch customer data for all unique customer IDs
  const fetchCustomerData = async (customerIds: string[]): Promise<CustomerDataMap> => {
    ...
    try {
      ...
      console.log(`🔄 Fetching ${customerPromises.length} customer(s) in parallel...`);
      ...
      customerResponses.forEach((response) => {
        ...
        customer.appointments.forEach((apt) => {
          ...
          console.log(`  📝 Mapped appointment ${apt.appointment_id}:`, serviceNames);
        });
  ...
  const formatAppointmentsForDisplay = (
    ...
    const formatted = appointments.map((apt) => {
      ...
      const customerName = customerMap[apt.customer_id]?.name || apt.customer_id;

      // Get services from map
      const services = customerMap[apt.customer_id]?.appointments[apt.appointment_id]?.services || [];
      const serviceString = services.length > 0 ? services.join(" + ") : apt.appointment_id;
  ...
  const fetchAppointmentsData = async () => {
    ...
    try {
      ...
      const baseUrl = "https://rizzerv.kloudwizards.com/gateway/search/salons/65426f3f-5784-48a1-9275-55c21039b80a/branches/eabc2278-e3ae-4606-877b-0d3830e2fb81";
      ...
      console.log('✅ Yesterday API Response received:', yesterdayResponse.data);

      // Calculate metrics
      console.group('🧮 Processing KPI Metrics');
      console.log('Today appointments count:', todayResponse.data.results.appointments?.length);
      console.log('Yesterday appointments count:', yesterdayResponse.data.results.appointments?.length);
      console.log('Today revenue:', todayResponse.data.results.totalRevenue);
      console.log('Today client visits:', todayResponse.data.results.clientVisits);
      console.log('Today utilization:', todayResponse.data.results.utilization);

      const metrics = calculateKPIMetrics(todayResponse.data, yesterdayResponse.data);
      ...
      const todayAppointmentsData = todayResponse.data.results.appointments || [];
      const uniqueCustomerIds = [...new Set(todayAppointmentsData.map((apt: BranchAppointment) => apt.customer_id))] as string[];
